...
## [Unreleased] - PRODUCT UX 2026-07

### 产品导航与任务

- **默认进入内容库**；工作台 `/home` 重定向到内容库。
- **移除文稿库页面**（`/transcripts` → 内容库）。阅读路径：内容库 → 创作者详情 → 已转写素材。
- **任务单一入口**：右下角状态球打开全高任务面板；去掉顶栏 ticker 与侧栏重复「任务中心」。
- 任务操作（暂停 / 停止 / 重试 / 删除）按钮加大、文案诚实：**暂停不支持断点续传**，继续会从头执行。
- 任务 pause/cancel/resume 后前端乐观更新状态并回拉接口，减少「点了没反应」。

### 内容库

- 卡片墙改为**表格列表**：历史收录 / 文稿 / 本地扫描 / 上次同步 / 自动跟进 / 操作。
- 添加创作者入口默认折叠；顶栏主操作为「添加」，次要操作收纳。
- **多选批量**：停跟并保留、彻底删除、增量同步、开/关自动跟进。
- **打开博主主页**（外链，新标签页）。
- **全部开启自动跟进**（`POST /api/v1/creators/auto-sync/bulk`）。
- 创作者列表默认 `limit=500`。

### 停跟 vs 彻底删除

- `DELETE /api/v1/creators/{uid}?keep_content=true`：停跟保留文稿（`sync_status=unfollowed`，`auto_sync=0`）。
- `keep_content=false`：删除创作者 + 素材记录 + 尽量清理本地目录。
- `POST /api/v1/creators/{uid}/refollow`：恢复已停跟。
- 删除确认弹窗改为二选一卡片；修复旧逻辑「仅删创作者」实际仍级联删素材的 bug。

### 转写阅读与其它

- docx 全文提取**保留段落换行**，修复在线阅读被压成一行的问题。
- 设置页内容区加宽（`max-w-4xl`）。
- 全量重拉统一强警告文案（与增量语义区分）。
- 创作者详情：视频 `archived` 状态展示；文稿文件 404 时说明外置归档。

### API 备注

- `GET /api/v1/transcripts` 默认 `availability=local`（仅本地文件存在）；前端已不再挂载文稿库页。
