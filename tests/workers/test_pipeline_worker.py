...
class PipelineWorkerTests(unittest.IsolatedAsyncioTestCase):
    async def test_run_pipeline_for_user_uses_threaded_download_path(self) -> None:
        ...
        with (
            patch("media_tools.download.service.download_by_url", download_mock),
            patch(
        ...
        self.assertEqual(
            mocked_to_thread.await_args.args[1:], ("https://www.douyin.com/user/test", 1, True, True, None)
        )
        ...
        update_progress.assert_any_await(
            0.4, "下载完成，准备转写 1 个视频...", "transcribe", {"download": {"done": 1, "total": 1}}
        )
