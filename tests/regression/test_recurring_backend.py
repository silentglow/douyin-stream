...
def test_no_bare_except_exception():
    ...
    result = subprocess.run(
        ["grep", "-rn", "except Exception:", "src/media_tools/"], capture_output=True, text=True, cwd=str(REPO_ROOT)
    )
    ...
    assert len(violations) == 0, f"发现 {len(violations)} 处宽泛异常捕获 (BACKEND-001):\n" + "\n".join(violations[:20])
...
def test_no_bare_sqlite3_connect():
    ...
    result = subprocess.run(
        ["grep", "-rn", "sqlite3.connect", "src/media_tools/"], capture_output=True, text=True, cwd=str(REPO_ROOT)
    )
    lines = [line for line in result.stdout.strip().split("\n") if line]
    allowed_paths = ("store/db.py",)
    [line for line in lines if any(p in line for p in allowed_paths)]
    violations = [line for line in lines if not any(p in line for p in allowed_paths)]
    assert len(violations) == 0, f"发现 {len(violations)} 处裸 sqlite3.connect (BACKEND-004):\n" + "\n".join(
        violations[:20]
    )
...
def test_managed_temp_file_unified():
    ...
    result = subprocess.run(
        ["grep", "-rln", "managed_temp_file", "src/media_tools/"], capture_output=True, text=True, cwd=str(REPO_ROOT)
    )
    ...
    for f in files:
        result2 = subprocess.run(
            ["grep", "-n", "def managed_temp_file", f], capture_output=True, text=True, cwd=str(REPO_ROOT)
        )
        if result2.stdout.strip():
            definition_files.append(f)
    assert len(definition_files) <= 1, f"managed_temp_file 在多处定义 (BACKEND-002/007): {definition_files}"
...
def test_modules_use_db_connection(module_name):
    ...
    result = subprocess.run(
        ["grep", "-n", "sqlite3.connect", f"src/{module_path}"], capture_output=True, text=True, cwd=str(REPO_ROOT)
    )
    lines = [line for line in result.stdout.strip().split("\n") if line]
    assert len(lines) == 0, f"{module_name} 仍使用裸 sqlite3.connect:\n" + "\n".join(lines)
...
def test_db_connection_leak_protection():
    ...
    sqlite3.connect(":memory:").execute("SELECT COUNT(*) FROM pragma_database_list()").fetchone()[0]
...
async def test_async_endpoint_not_blocking_event_loop():
    ...
    # 给一些裕量
    assert elapsed < 0.8, f"操作疑似串行执行，耗时 {elapsed:.2f}s (BACKEND-005)"
...
def test_async_db_wrapper_exists():
    ...
    has_async = any(name.startswith("a") for name in dir(core) if callable(getattr(core, name, None))) or hasattr(
        core, "AsyncDBConnection"
    )
...
def test_pragma_table_info_uses_validation(router):
    ...
    for line in lines:
        ...
        if 'f"PRAGMA table_info' in line or "f'PRAGMA table_info" in line:
            pytest.fail(f"{router}.py 存在无校验的 PRAGMA f-string: {line} (BACKEND-006)")
...
def test_get_table_columns_unified():
    ...
    files = [line.strip() for line in result.stdout.strip().split("\n") if line.strip()]
    assert len(files) <= 1, f"_get_table_columns 在多处定义 (BACKEND-006/007): {files}"
...
def test_workers_do_not_update_task_status_via_raw_sql():
    ...
    assert not violations, "workers 不应直接写 task_queue.status (BACKEND-008):\n" + "\n".join(violations)
...
def test_pragma_table_info_only_in_db_core():
    ...
    violations = [line for line in lines if not any(p in line for p in allowed_paths)]
    assert not violations, "PRAGMA table_info 应只出现在 db/core.py 或 store/db.py (BACKEND-009):\n" + "\n".join(
        violations
    )
...
def test_no_duplicate_managed_temp_file():
    ...
    for func in funcs:
        ...
        result = subprocess.run(
            ["grep", "-rln", f"def {func}", "src/media_tools/"], capture_output=True, text=True, cwd=str(REPO_ROOT)
        )
        files = [line.strip() for line in result.stdout.strip().split("\n") if line.strip()]
        assert len(files) <= 1, f"{func} 在多处定义 (BACKEND-007): {files}"
