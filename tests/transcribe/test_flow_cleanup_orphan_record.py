...
@pytest.mark.asyncio
async def test_upload_failure_triggers_orphan_record_cleanup(
    ...
    upload = AsyncMock(side_effect=RuntimeError(
    ...
    with patch.object(flow_mod, "api_json", api_json), \
    ...
    assert args[1] == ["orphan-record-id-12345"], (
        f"delete_record 必须被调用以清理孤儿 recordId,实际参数 {args}"
    )


@pytest.mark.asyncio
async def test_export_failure_also_triggers_cleanup(
    video_file: Path, tmp_path: Path
) -> None:
    ...
    with patch.object(flow_mod, "api_json", api_json), \
        ...
        with pytest.raises(RuntimeError, match="export failed at qwen side"):
            await run_real_flow(
                file_path=video_file,
                auth_state_path=tmp_path / "auth.json",
                download_dir=tmp_path / "out",
...
async def test_cleanup_swallows_its_own_failure_to_preserve_original_exception(
    ...
    with patch.object(flow_mod, "api_json", api_json), \
        ...
        with pytest.raises(RuntimeError, match="ORIGINAL_UPLOAD_ERROR"):
            await run_real_flow(
                file_path=video_file,
                auth_state_path=tmp_path / "auth.json",
                download_dir=tmp_path / "out",
